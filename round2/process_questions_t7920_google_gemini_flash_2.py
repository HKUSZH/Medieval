...
# Configuration
client = genai.Client(api_key="AIzaSyDwBIdc_k3CIiK1Ev0**********iXH0-0")
...
def process_questions(questions, output_file):
    ...
    for i, question in enumerate(tqdm(questions, desc="Processing questions")):
        if i < 500:
            continue
        qID = question["QID"]
        thisPrompt = question["text"]
        print(f"question [{i}]\t[{time.strftime('%Y-%m-%d %H:%M:%S')}]\tQID=[{qID}]")
...
def main():
    ...
    output_file = "responses_google_Gemini_2_Flash_think.json"
