...
# 0 初始化 Qwen 客户端
client = AsyncOpenAI(
    api_key='sk-7a6889bec553448ba4**********e778',  #
    base_url="https://dashscope.aliyuncs.com/compatible-mode/v1"
)

# 1 保存文件
def save_file(my_list,file_name_index,default_name='./all_solutions_qwen_max/solutions_'):
    folder_name='./all_solutions_qwen_max'
...
async def rate_limited_batch(prompts, batch_size=5):
    for i in range(0, len(prompts), batch_size):
        ...
        if i + batch_size < len(prompts):  # 确保在不是最后一批时等待
            ...
            print("等待 20 秒后，开始下一个批次...")
            await asyncio.sleep(20)  # 每执行完一批后等待 10 秒
            print('-----------------------------------------------------')
        else:
            print('最后一批结束')
...
async def batch_chat(prompts):
    tasks = [call_qwen(prompt) for prompt in prompts]
...
# 4 依次读取所有文件,合并为1张表
import re
folder_path='./all_solutions_qwen_max'
...
# 保存到本地
all_solutions_qwen_max=[]

for i in range(len(merged_data)):
    # 将问题和回答保存在最终的结构中
    all_solutions_qwen_max.append({
        'question': source_data[i],
        'response': merged_data[i]
    })

with open("all_solutions_qwen_max.json", "w", encoding="utf-8") as file:
    json.dump(all_solutions_qwen_max, file, ensure_ascii=False, indent=4)

print("结果已保存为all_solutions_qwen_max.json")
