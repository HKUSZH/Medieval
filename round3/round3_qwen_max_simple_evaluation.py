...
# 'deptAnswers.01.AE_shuffled.json'

#  replace
# 'qwen_max_2025.rate.01.AE_shuffled.json'
...
client = OpenAI(
    api_key="sk-**********53448ba449a22b1f9ee778",
...
def read_department(output_file):
    pattern =  r"qwen_max_2025\.rate\.\d+\.(.*?)_shuffled\.json"
    match = re.search(pattern, output_file)
    return match.group(1)


def save_incremental_results(results, output_file, batch_number):
    # output_file 'qwen_max_2025.rate.01.AE_shuffled.json'
    # batch_file  'qwen_batch_33.json'
    department=read_department(output_file)
    folder_name = './all_solutions_qwen/'+department
...
def save_results(results, output_file):
    ...
    folder_name = './all_solutions_qwen_final'
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
...
def main():
    ...
    for input_file in filtered_files:
        ...
        output_file = input_file.replace("deptAnswers", "qwen_max_2025.rate")
        # output_file 'qwen_max_2025.rate.01.AE_shuffled.json'
