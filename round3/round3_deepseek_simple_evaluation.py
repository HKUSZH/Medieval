...
client = OpenAI(
    api_key="sk-**********ec48f1a66dc20e5aed166e",
...
def read_department(output_file):
    pattern =  r"deepseek_671\.rate\.\d+\.(.*?)_shuffled\.json"
...
def save_incremental_results(results, output_file, batch_number):
    ...
    folder_name = './all_solutions_deepseek/'+department
...
def save_results(results, output_file):
    ...
    folder_name = './all_solutions_deepseek_final'
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
...
def main():
    ...
    for input_file in filtered_files:
        ...
        output_file = input_file.replace("deptAnswers", "deepseek_671.rate")
        # output_file 'deepseek_671.rate.01.AE_shuffled.json'
